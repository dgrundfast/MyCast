...
async function getReqUser(req) {
  ...
  const u = rows[0];
  if (DEVELOPER_IDS.includes(u.id)) u.tier = 'paid';
  return u;
}
...
const ttsLimit = makeLimiter(1);  // 1 concurrent TTS call prevents OpenAI 429s
const genLimit = makeLimiter(3);
...
async function writeSegmentScript(topic, sources, targetMin) {
  ...
  const prompt =
    ...
    'HOW TO REPORT:\n' +
    '- START WITH A FACT. First sentence names a subject and states a concrete development.\n' +
    '- INVERTED PYRAMID: the most important development first, supporting detail in descending order.\n' +
    '- SYNTHESIZE ACROSS SOURCES: never summarize a single article.\n' +
    ...
    '- NEVER PAD. If the news genuinely runs short, write less.\n\n' +
...
/* =============================================================================
   ...
   ========================================================================== */
const TTS_LEXICON = {
  meme: 'meem', memes: 'meems', thai: 'tie', thais: 'ties',
...
async function ttsOpenAI(text, voice) {
  ...
  for (let i = 0; i < maxTries; i++) {
    try {
      ...
      clearTimeout(t);
      if (r.status === 429) {
        // Rate limited — exponential backoff: 5s, 10s, 20s, 40s
        const wait = 5000 * Math.pow(2, i);
        console.log('TTS 429 rate limit, waiting ' + (wait/1000) + 's before retry ' + (i+1) + '/' + maxTries);
      ...
      lastErr = e;
      if (i < maxTries - 1) await new Promise(res => setTimeout(res, 2000 * (i + 1)));
    }
  }
  throw lastErr || new Error('TTS failed after ' + maxTries + ' attempts');
}
async function synthesizeToFile(text, voice, outPath) {
  ...
  // BUG 1 FIX: Byte-based duration (OpenAI tts-1 ~32kbps = 4000 bytes/sec)
  // Word-count estimation drifted up to 36s on real files.
...
async function generateTopic(topic, date, activeVoices, globalUsedTitles) {
  ...
  try {
    ...
    // BUG 3 FIX: Content-address filenames with script hash so re-renders get new URLs.
...
async function runBatch() {
  ...
  // Pre-generate all transition audio so assembly never hits TTS on-demand.
  // Sequential (not parallel) to avoid 429s after an already heavy batch.
  console.log('Pre-generating transition audio...');
  const { rows: segs } = await pool.query(
    `SELECT DISTINCT topic_id, voice,
       (SELECT label FROM topics t WHERE t.id = segments.topic_id) AS label
     FROM segments WHERE cycle_date=$1 AND status='ok'`, [date]);
  let transOk = 0, transFail = 0;
  for (const s of segs) {
    if (!s.label) continue;
    try {
      await ensureTransitionAudio(s.label, s.voice, date);
      transOk++;
    } catch (e) {
      console.log('transition pre-gen failed [' + s.label + '/' + s.voice + ']:', e.message);
      transFail++;
    }
  }
  console.log('Transitions: ok=' + transOk + ' failed=' + transFail);
...
async function assembleBrief(brief, date) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    // BUG 2 FIX: Cap play_sec to (duration_sec - 2) to avoid mid-sentence cuts.
    items.push({
      topic_id: s.topic_id,
      label: s.label,
  ...
  const introText = firstName
    ...
    : 'Here\'s your Cast. ' + weekday + '.';
  const manifest = {
    date, voice: primaryVoice, voice_requested: requestedVoice,
...
async function resolveOrCreateTopic(rawLabel) {
  ...
  if (rows[0]) return { topic: rows[0], created: false };
...
app.post('/api/brief/custom-topic', requireUser(async (req, res) => {
  ...
  res.json({
    ok: true, topic: { id: topic.id, label: topic.label }, created,
...
app.post('/api/admin/batch/run', requireAdmin(async (_req, res) => {
  ...
  runBatch().catch(e => console.error('batch error:', e));
}));
    ...
    if (b.rows[0]) {
      ...
      const seg = await pool.query(
        `SELECT topic_id, voice, cycle_date::text as cycle_date, duration_sec, status
           FROM segments WHERE topic_id = ANY($1) AND status='ok'
